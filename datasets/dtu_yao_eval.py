...
# NOTE: to avoid dataloader error need to set opencv thread as 0
cv2.setNumThreads(0)
...
class MVSDataset(Dataset):
    ...
    def read_img_hr(self, filename):
        # NOTE: (640,512,3)->(3,640,512), (3,320,256), (3,160,128)
        img = Image.open(filename)
        np_img = np.array(img, dtype=np.float32)

        h, w = np_img.shape[:2]
        stage1 = cv2.resize(np_img, (w//4, h//4), interpolation=cv2.INTER_AREA)
        stage1 = stage1.transpose([2, 0, 1])
        stage2 = cv2.resize(np_img, (w//2, h//2), interpolation=cv2.INTER_AREA)
        stage2 = stage2.transpose([2, 0, 1])
        stage3 = np_img.transpose([2, 0, 1])
        np_img_ms = {
            "stage1": stage1,
            "stage2": stage2,
            "stage3": stage3,
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            img_filename = os.path.join(self.datapath, 'dtu_noise', '{}/images_post/{:0>8}.jpg'.format(scan, vid))
            if not os.path.exists(img_filename):
                img_filename = os.path.join(self.datapath, 'dtu_noise', '{}/images/{:0>8}.jpg'.format(scan, vid))
            img_clean_filename = os.path.join(self.datapath, 'dtu', '{}/images/{:0>8}.jpg'.format(scan, vid))
            ...
            img_clean, _ = self.scale_mvs_input(img_clean, intrinsics_copy, self.max_w, self.max_h) 
            # if transfer intrinsics here, the source intrinsics will be changed
            if self.fix_res:
            ...
            if i == 0:  # reference view
                img_clean_ms = self.read_img_hr(img_clean_filename)
                depth_values = np.arange(depth_min, depth_interval * (self.ndepths - 0.5) + depth_min, depth_interval,
                                         dtype=np.float32)

        #all
        imgs = np.stack(imgs).transpose([0, 3, 1, 2])
        imgs_clean = np.stack(imgs_clean).transpose([0, 3, 1, 2])
        ...
        return {"imgs": imgs,
                "imgs_clean": imgs_clean, # imgs_clean are the same number of imgs, but they are clean, imgs are noise
                "img_clean" : img_clean_ms, # img_clean is multiscale ref clean img
