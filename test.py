...
parser.add_argument('--refine', default=False, help='choose whether add refine module')
parser.add_argument('--imgs', default='clean' , choices=['clean', 'noise', 'net'], help='choose which images to be reconstruct to ply file')
...
def save_depth():
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            imgs_clean = sample["imgs_clean"].numpy()

            # save depth maps and confidence maps
            for filename, cam, img, img_clean, depth_est, photometric_confidence in zip(filenames, cams, imgs, imgs_clean,\
                                                                              outputs["depth"], outputs["photometric_confidence"]):
                # TODO the plyfile generated base on which imgs:(clean, noise, net)
                # img = img[0]  #ref view
                img = img_clean[0] # clean ref view
                cam = cam[0]  #ref cam
...
def filter_depth(scan_folder, out_folder, plyfilename, sacn):
    # the pair file
    ...
    for ref_view, src_views in pair_data:
        ...
        # hanjiawei: the clean imgs generated by net
        # if args.imgs == 'net':
        #     ref_img = read_img(os.path.join(args.outdir, sacn, 'rgb/{:0>8}.jpg'.format(ref_view)))
        # else:
        ...
        print("processing {}, ref-view{:0>2}, photo/geo/final-mask:{}/{}/{}".format(out_folder, ref_view,
                                                                                    photo_mask.mean(),
...
if __name__ == '__main__':  
    ...
    args.loadckpt = '/home/ubuntu/hjw/Code_DEMVSNet/model/CasMVSNet+Ours.ckpt' # Select the model to be tested
    ...
    params_list = product([args.testpath], [args.outdir], scans)

    """
    for scan in scans:
        scan_id = int(scan[4:])
        
        # NOTE:the img used to reconstruct ply file need to be clean ,so choose the clean images or the images generated by net
        if args.imgs == 'clean':
            # scan_folder = os.path.join(os.path.dirname(args.testpath), 'dtu', scan)
            scan_folder = os.path.join(args.testpath, 'dtu', scan)
        elif args.imgs == 'noise':
            scan_folder = os.path.join(args.testpath, 'dtu_noise', scan)
        elif args.imgs == 'net': 
            scan_folder = os.path.join(args.testpath, 'dtu', scan)
        
        out_folder = os.path.join(args.outdir, scan)
        # step2. filter saved depth maps with photometric confidence maps and geometric constraints
        filter_depth(scan_folder, out_folder, os.path.join(args.outdir, 'mvsnet{:0>3}_l3.ply'.format(scan_id)), scan)
    """

    # # NOTE: to multiprocesss args.testpath, args.outdir need to be hardcode seted
    ...
    #     # NOTE:the img used to reconstruct ply file need to be clean ,so choose the clean images or the images generated by net
    #     # if args.imgs == 'clean':
    #     #     scan_folder = os.path.join(params[0], 'dtu', scan)
    #     # elif args.imgs == 'noise':
    #     #     scan_folder = os.path.join(params[0], 'dtu_noise', scan)
    #     # elif args.imgs == 'net': 
    #     #     scan_folder = os.path.join(params[0], 'dtu', scan)
    #     scan_folder = os.path.join(params[0], 'dtu', scan)

    #     out_folder = os.path.join(params[1], scan)
    #     # step2. filter saved depth maps with photometric confidence maps and geometric constraints
    #     filter_depth(scan_folder, out_folder, os.path.join(params[1], 'mvsnet{:0>3}_l3.ply'.format(scan_id)), scan)
